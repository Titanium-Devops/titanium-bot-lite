...
| 6 | `GET /api/models` | — | `{live:{source:"device"\|"lan"\|"cloud", endpoint, model}, device:[{id,name,running}], lan:[{baseUrl,model}], note}` |
| 7 | `POST /api/model` | `{action:"use"\|"start"\|"stop", id}` or `{action:"use", baseUrl, model, apiKey}` | `{live:{…}}`. The key goes to the 0600 file and is **never echoed** |
